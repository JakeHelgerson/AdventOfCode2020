import Day2.{Line, rawData}

import scala.util.{Failure, Success, Try}

class Day2 {

  val data: Seq[Line] = rawData
    .split("\n")
    .map(string =>  {
      val pattern = """(\d+)-(\d+)\s(.):\s(.+)""".r
      val pattern(lower, upper, letter, password) = string
      Line(lower.toInt, upper.toInt, letter.charAt(0), password)
    })
...
object Day2 {
  ...
  case class Line(lower: Int, upper: Int, letter: Char, password: String)

  val rawData: String = """5-10 b: bhbjlkbbbbbbb
    |3-4 j: hjvj
    |8-9 p: pmljtsttp
